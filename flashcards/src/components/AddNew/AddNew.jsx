...
function AddNew() {
  ...
  const saveEdit = () => {
    ...
    if(newEnglish === '' || newTranscription === '' || newRussian === '' || newTags === '' ){
      setIsValid(false);
    } else {
      setIsValid(true);
      setPressed(!pressed);
  ...
  return (
    <>
      ...
      {pressed ? (<div className='wordItem'>
        <input className={isValidValueEnglish? 'word_input' : 'word_input red'} name='english' value={newEnglish} onChange={handleChangeEng} />
        <input className={isValidValueTranscription? 'word_input' : 'word_input red'} name='transcription' value={newTranscription} onChange={handleChangeTrans} />
        <input className={isValidValueRussian? 'word_input' : 'word_input red'} name='russian' value={newRussian} onChange={handleChangeRus} />
        <input className={isValidValueTags? 'word_input' : 'word_input red'} name='tags' value={newTags} onChange={handleChangeTags} />
        <button className='button save' onClick={saveEdit} ></button>
        <button className='button cancel' onClick={cancelEdit}></button>
      </div>) : (<div className='button new' onClick={handleClick}></div>
      )}
    </>
  )
}
